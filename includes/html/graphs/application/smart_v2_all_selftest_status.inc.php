...
// Self-test status state per disk, relabeled per the saved naming template /
// label mode, same approach as the all-temperatures overview graph. State
// codes are plotted as raw numbers, same as the all-health overview graph.
...
if ($htmlData !== null) {
    foreach ($htmlData->diskKeys() as $diskKey) {
        $sensor = $htmlData->selftestStatusSensor($diskKey);
        if ($sensor === null) {
            continue;
        }
