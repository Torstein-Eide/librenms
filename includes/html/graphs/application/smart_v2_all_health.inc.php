...
// SMART overall-health self-assessment state per disk, relabeled per the
// saved naming template / label mode, same approach as the all-temperatures
// overview graph. State codes are plotted as raw numbers (see currentTranslation()
// for the state_descr -> code mapping used by the per-disk health badge).
...
if ($htmlData !== null) {
    foreach ($htmlData->diskKeys() as $diskKey) {
        $sensor = $htmlData->healthSensor($diskKey);
        if ($sensor === null) {
            continue;
        }
