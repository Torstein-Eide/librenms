...
// Hours since last Extended (long) self-test per disk, relabeled per the
// saved naming template / label mode, same approach as the
// all-temperatures overview graph.
...
if ($htmlData !== null) {
    foreach ($htmlData->diskKeys() as $diskKey) {
        $sensor = $htmlData->selftestAgeSensor($diskKey, 'long');
        if ($sensor === null) {
            continue;
        }
