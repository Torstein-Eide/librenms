...
// TODO: Upgrade this module to use the new Application framework properly:
// - Replace $app->data / update_application() with Application::saveAppData() / saveMetrics()
// - Replace app('Datastore')->put() with Application::storeRrd()
// - Replace manual Eventlog::log() alerts with Application sensor thresholds
// - Create sensors via Application::discoverSensor() for health, temp, wear, self-test age
// - Store per-disk state in dedicated DB tables (like mdadm) instead of $app->data blobs
// - Replace json_app_get() with $this->agent_data passed by the framework
// - Remove $device array dependency; use $this->os->getDevice() or $this->os->getDeviceId()
// - Wire discover() to sync sensors; poll() to update values only

class SmartV1 extends Application
{
