 ...
 *   - Constructor replaced by parent's (OS $os, ApplicationModel $app, array $agent_data)
 *   - $this->device  →  $this->os->getDevice()
 *   - $this->payload →  $this->agent_data
 *   - run()          →  discover() + poll()  (called separately by the framework)
 *   - shouldDiscover() gates discovery via the tick counter
 *
 * No logic changes.
 */
class SmartV2 extends Application
{
    ...
    {
        $disks = $this->agent_data['data']['tables']['disks'] ?? [];

        if (empty($this->discovery)) {
            return true; // first poll, run discovery
        }

        $currentCount = count($disks);
        if ($currentCount > ($this->discovery['disk_count'] ?? 0)) {
            return true; // new disk added, run immediately
        }
        ...
        if (($this->discovery['tick'] ?? 0) < $this->DISCOVERY_INTERVAL) {
            ...
            return false; // interval not reached, skip
        }

        return true; // interval reached, run
    }
    ...
    public function poll(): void
    {
        ...
        $counters = $this->agent_data['data']['counters'] ?? [];
    ...
    private function pollDiskRrd(string $diskKey, array $disk): void
    {
        ...
        if (is_array($nvmeLog)) {
            ...
            app('Datastore')->put($device, 'app', $tags_nvme, $fields_nvme);
        }
        ...
        if ($tempC !== null || $hours !== null || $cycles !== null) {
            ...
            app('Datastore')->put($device, 'app', $tags_pwr, $fields_pwr);
        }
    }
    ...
    private function updateSensors(string $diskKey, array $disk): void
    {
        ...
        if ($sensor = $sensors->get("{$idx}_health")) {
            $value = isset($disk['health']['smart_passed'])
                ? ($disk['health']['smart_passed'] ? 0 : 1)
                : null;
            $this->updateStateSensor($device, $sensor, $value);
        }
        ...
        if ($sensor = $sensors->get("{$idx}_nvme_crit_warn")) {
            ...
            $this->updateStateSensor($device, $sensor, $cwRaw !== null ? (int) $cwRaw : null);
        }
        ...
        if (($sensor = $sensors->get("{$idx}_wear")) && ! isset($this->discovery['disk_list'][$diskKey]['sensors']["{$idx}_wear"])) {
            $this->updateNumericSensor($device, $sensor, $this->extractWear($disk));
        }

        // Self-test ages (computed)
        if ($sensor = $sensors->get("{$idx}_selftest_short")) {
            $age = $this->hoursSinceTest($disk, 'short');
            $this->updateNumericSensor($device, $sensor, $age !== null ? (float) $age : null);
        }
    ...
    private function updateNumericSensor(array $device, Sensor $sensor, ?float $sensor_value): void
    {
        ...
        app('Datastore')->put($device, 'sensor', $tags, ['sensor' => $sensor_value]);
    }

    private function updateStateSensor(array $device, Sensor $sensor, ?int $value): void
    {
        $sensorValue = $value ?? -1;
        ...
        app('Datastore')->put($device, 'sensor', $tags, ['sensor' => $sensorValue]);
    }
